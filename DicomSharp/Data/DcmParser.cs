    ...
    public class DcmParser {
        ...
        private FileFormat DetectFileFormat(Stream ins) {
            ...
            try {
                ...
                if (pos != 128 || m_ins.ReadByte() != 'D' || m_ins.ReadByte() != 'I' || m_ins.ReadByte() != 'C' ||
                    m_ins.ReadByte() != 'M') {
        ...
        public long ParseItemDataset() {
            ...
            rPos += 8;
            var itemtag = (uint) ((bb12.ReadInt16(0) << 16) | (bb12.ReadInt16(2) & 0xffff));
        ...
        private long ParseItem(int id, int vr, int itemlen) {
            ...
            switch (vr) {
                ...
                default:
                    throw new SystemException(LogMsg());
            }
        }
