...
namespace Dicom.Net {
    ...
    public class BasicSCU {
        ...
        private static readonly AssociationFactory aFact = AssociationFactory.Instance;
        private static readonly DcmObjectFactory oFact = DcmObjectFactory.Instance;
        private static readonly DcmParserFactory pFact = DcmParserFactory.Instance;

        private static readonly String[] DEF_TS = new[] {UIDs.ImplicitVRLittleEndian};

        private readonly AAssociateRQ m_assocRQ = aFact.NewAAssociateRQ();
        private readonly int m_assocTimeOut;
        private readonly String m_host = "localhost";
        private readonly int m_port = 104;
        ...
        public BasicSCU(String callingAET, String calledAET, String host, int port, int timeout)
            : this(callingAET, calledAET, host, port) {
        ...
        public Dataset[] CFindStudy(string patid, string[] modalities) {
            ...
            try {
                ...
                m_assocRQ.AddPresContext(aFact.NewPresContext(pcid, sopClassUID, new[] {m_tsUID}));
                ActiveAssociation active = OpenAssoc();
                if (active != null) {
                    var ds = new Dataset();

                    var fmi = new FileMetaInfo();
                    ...
                    else if ((pc = assoc.GetAcceptedPresContext(sopClassUID, UIDs.ImplicitVRLittleEndian)) == null &&
                             (pc = assoc.GetAcceptedPresContext(sopClassUID, UIDs.ExplicitVRLittleEndian)) == null &&
                             (pc = assoc.GetAcceptedPresContext(sopClassUID, UIDs.ExplicitVRBigEndian)) == null) {
                        log.Error("SOP class UID not supported");
                        return null;
                    }

                    Dimse rq = aFact.NewDimse(pcid, oFact.NewCommand().InitCFindRQ(assoc.NextMsgID(), sopClassUID, 0),
                                              ds);
        ...
        public Dataset WADOGet(string url, string studyInstanceUID, string seriesInstanceUID, string sopInstanceUID) {
            ...
            try {
                ...
                sb.Append(HttpUtility.UrlEncode(studyInstanceUID));
                sb.Append("&seriesUID=");
                sb.Append(HttpUtility.UrlEncode(seriesInstanceUID));
                sb.Append("&objectUID=");
                sb.Append(HttpUtility.UrlEncode(sopInstanceUID));
                ...
                if (string.Compare(response.ContentType, "application/dicom") == 0) {
                    var parser = new DcmParser(response.GetResponseStream());
            ...
            if (parser != null) {
                ...
                return
                    active.Invoke(aFact.NewDimse(pc.pcid(),
                                                 oFact.NewCommand().InitCStoreRQ(assoc.NextMsgID(), sopClassUID,
                                                                                 sopInstUID, 0),
                                                 new FileDataSource(parser, ds, new byte[2048])));
            }
            else {
                if ((ds.GetFileMetaInfo() != null)
                    && (ds.GetFileMetaInfo().TransferSyntaxUID != null)) {
