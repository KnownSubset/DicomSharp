...
namespace Dicom.Dictionary {
    public class Status {
        ...
        private static readonly Hashtable s_dict = new Hashtable(50);

        static Status() {
            FieldInfo[] fields = typeof (Status).GetFields(BindingFlags.Public | BindingFlags.Static);
            foreach (FieldInfo field in fields) {
                String name = field.Name;
                var status = (int) field.GetValue(null);
        ...
        public static String GetName(int status) {
            if (s_dict.Contains(status)) {
                ...
                throw new ArgumentException("Unkown status: " + ToHexString(status));
            }
        }
        ...
        public static void Main() {
            try {
                Console.WriteLine("OptionalKeysNotSupported=" + ToHexString(GetStatus("OptionalKeysNotSupported")));
                Console.WriteLine("0xFF01=" + GetName(0xFF01));
            }
            catch (Exception e) {
                Console.WriteLine(e);
